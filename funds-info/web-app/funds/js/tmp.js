//save from http://data.eastmoney.com/zjlx/dpzjlx.html
// http://m.data.eastmoney.com/zjlx/index

// https://ff.eastmoney.com/EM_CapitalFlowInterface/api/js?id=ls&type=ff&check=MLBMS&cb=var%20aff_data=&js={(x)}&rtntype=3&acces_token=1942f5da9b46b069953c873404aad4b5
// https://ff.eastmoney.com/EM_CapitalFlowInterface/api/js?id=ls&type=ff&check=MLBMS&cb=var%20aff_data=&js={(x)}&rtntype=3&acces_token=1942f5da9b46b069953c873404aad4b5
    ...
    tiny.loadJs(b, "utf-8", function () {
        if (!(typeof tab_data == "undefined" || tab_data == null)) {
            ...
            if (dataArr.length == 2) {
                ...
                data[0] = (_dh[5] == "" || isNaN(parseFloat(_dh[5]))) ? "0.0000" : ((parseFloat(_dh[5]) + parseFloat(_ds[5])) / 10000).toFixed(4);//今日主力净流入
    ...
    tiny.loadJs(b, "utf-8", function () {
        if (!(typeof tab_data == "undefined" || tab_data == null)) {
            ...
            if (dataArr.length == 1) {
                ...
                //算和值
                var data = [];
                data[0] = (_dh[5] == "" || isNaN(parseFloat(_dh[5]))) ? "0.0000" : ((parseFloat(_dh[5])) / 10000).toFixed(4);//今日主力净流入
...
function hqUpdate() {
    ...
    var url = "http://nufm.dfcfw.com/EM_Finance2014NumericApplication/JS.aspx?type=CT&cmd=P.(x),(x)|0000011,3990012|0000011&sty=DCFFBR|UDFN&st=z&sr=1&p=1&ps=&cb=&js=var%20hsHq=[(x)]&token=1942f5da9b46b069953c873404aad4b5";
    ...
    tiny.loadJs(b, "utf-8", function () {
        if (!(typeof hsHq == "undefined" || hsHq == null)) {
            c = hsHq[0].split(",");
            if (c.length < 8) {
                c = ["-", "-", "-", "-", "-", "-", "-"]
            }
            type = e(c[5], 0);
            d = spanCache.replace(/{dqzs}/ig, c[3]);
            d = d.replace(/{zdqs}/ig, type == "green" ? " <span class=\"arr\">↓</span>" : " <span class=\"arr\">↑</span>");
            d = d.replace(/{zdzs}/ig, c[4]);
            d = d.replace(/{zdf}/ig, c[5]);
            d = d.replace(/{cje}/ig, c[6]);
            $("shhq").className = type;
            $("shhq").innerHTML = d;

            var hszd = hsHq[2].split(",");
            var zdCount = hszd[0].split("|");//涨跌数量
            $("shz").innerHTML = zdCount[0];
            $("shp").innerHTML = zdCount[1];
            $("shd").innerHTML = zdCount[2];
            ...
            zdCount = hszd[1].split("|");//涨跌数量
            $("szhq").innerHTML = d;
            $("szz").innerHTML = zdCount[0];;
            $("szp").innerHTML = zdCount[1];;
            $("szd").innerHTML = zdCount[2];;
        }
    }, true)
}
